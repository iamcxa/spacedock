...
- 🔴 Unclear (0.20-0.49) -- weak signal, likely needs reclassification

**Clarify presentation rule**: When SO presents assumptions to captain during clarify, each assumption MUST be expanded to a full-text block with: statement, emoji + confidence reasoning, evidence (file:line), and implementation implications ("實作意義"). Do NOT compress assumptions into AskUserQuestion option labels -- the UI truncates critical detail the captain needs to evaluate.
