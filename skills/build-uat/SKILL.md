...
**Routing for infra-level fails.** Append result to scratch with type `fail-infra`. Infra-level fails short-circuit the captain phase: if any item is `fail-infra`, you skip Step 4 entirely for that item AND (if ANY item is `fail-infra`) you will terminate the UAT run at Step 5 with `feedback-to: execute`. The captain phase still runs for the surviving assertion fails only if the orchestrator judges them worth sign-off -- the default is to consolidate routing. If ALL fails are infra-level, skip Step 4 entirely and write the Stage Report at Step 6 with `feedback-to: execute`.

**Routing for assertion fails.** Append to scratch with type `fail-assertion`. These continue to Step 4 for captain review with full evidence (screenshots, diffs, raw output). Captain decides per item: retry / override (mark pass with reason) / feedback (route back to execute with notes).
