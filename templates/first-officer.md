...
3. **Create team** — Derive the project name from `basename $(git rev-parse --show-toplevel)` and the directory basename from the workflow directory path. Probe for team support: `ToolSearch(query="select:TeamCreate", max_results=1)`. If the result contains a TeamCreate definition, find a free team name: set `base_name="{project_name}-{dir_basename}"` and check whether `~/.claude/teams/{base_name}/` exists. If it does, try `{base_name}-2`, `{base_name}-3`, etc. until finding a name whose directory does not exist. Run `TeamCreate(team_name="{free_name}")` with the first available name. **NEVER delete existing team directories** — stale directories belong to other sessions. If ToolSearch returns no match, enter **bare mode**: report the following to the captain and skip TeamCreate:
