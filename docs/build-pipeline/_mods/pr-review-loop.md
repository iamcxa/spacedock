...
- **Relationship to `pr-merge.md`**: This mod is the advanced PR lifecycle mod with review closed-loop. `mods/pr-merge.md` remains in the library as a simpler alternative for workflows that do not need review triage. Workflows activate whichever mod fits their needs via `{workflow_dir}/_mods/`.
