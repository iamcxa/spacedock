...
3. **Check concurrency** — Count how many tasks currently have their status set to the target stage. If the count equals the concurrency limit, hold this task in its current stage and move to the next dispatchable task.
4. **Conflict check** — When multiple tasks are entering a worktree stage simultaneously, check if they modify the same files. If so, warn CL about potential merge conflicts and propose sequencing them.
...
7. **Merge to main** — Only when the task has reached its terminal stage AND was in a worktree:
   ```bash
...
4. **Verify state** — Run `bash docs/plans/status` to confirm the task's status on disk.
5. **Dispatch next** — Look at the updated pipeline state. If any other task is ready for its next stage, dispatch an ensign for it (following the full dispatch procedure). Prioritize by score (highest first) when multiple tasks are ready.
6. **Repeat** — Continue until no tasks are ready for dispatch (all are in the terminal stage, blocked by approval gates, at concurrency limit, or the pipeline is empty).
