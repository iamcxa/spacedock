...
describe("evolve — gate_approved", () => {
  test("adds decision to empty state", () => {
    ...
    const event: GateEvent = { type: "gate_approved", entitySlug: "e1", stage: "plan", decidedBy: "captain", decidedAt: NOW };
...
describe("evolve — gate_rejected", () => {
  test("adds rejection with reason", () => {
    const state: GateState = new Map();
    const event: GateEvent = { type: "gate_rejected", entitySlug: "e1", stage: "plan", decidedBy: "captain", reason: "bad plan", decidedAt: NOW };
    const next = evolve(state, event);
    const snap = next.get("e1::plan");
...
describe("replay", () => {
  test("rebuilds state from event sequence", () => {
    const events: GateEvent[] = [
      { type: "gate_approved", entitySlug: "e1", stage: "plan", decidedBy: "captain", decidedAt: NOW },
      { type: "gate_rejected", entitySlug: "e2", stage: "uat", decidedBy: "captain", reason: "fails", decidedAt: NOW + 1 },
    ];
