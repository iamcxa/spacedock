...
describe("shutdown handler", () => {
  test("disconnectAll called with shutdown reason for all active sessions", async () => {
    ...
    await registry.register({ sessionId: "sess-1", projectRoot: "/repo-a", pid: 1, protocolVersion: 1 });
    await registry.register({ sessionId: "sess-2", projectRoot: "/repo-b", pid: 2, protocolVersion: 1 });
    ...
    const monitor: HeartbeatMonitor = {
      start: () => {},
      stop: () => { monitorStopped = true; callLog.push("monitor.stop"); },
    };

    const watcher: FileWatcher = {
      recomputeScope: async () => {},
      close: () => { watcherClosed = true; callLog.push("watcher.close"); },
    };
  ...
  test("teardown order: monitor.stop → watcher.close → disconnectAll", async () => {
    ...
    const monitor: HeartbeatMonitor = {
      start: () => {},
      stop: () => { callOrder.push("1-monitor.stop"); },
    };
    const watcher: FileWatcher = {
      recomputeScope: async () => {},
      close: () => { callOrder.push("2-watcher.close"); },
    };
    const wrappedRegistry = {
      ...registry,
